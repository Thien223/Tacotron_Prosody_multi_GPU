...
from Utils.Utils import waveplot
...
class Synthesizer:
    ...
    def synthesize(self, mel_spectrograms, speaker_ids, basenames, out_dir, log_dir):
        ...
        # [-max, max] or [0,max]
        T2_output_range = (
        -self._hparams.max_abs_value, self._hparams.max_abs_value) if self._hparams.symmetric_mels else (
        0, self._hparams.max_abs_value)
        c_batch = np.stack([_pad_inputs(x, maxlen, _pad=T2_output_range[0]) for x in mel_spectrograms]).astype(
            np.float32)
