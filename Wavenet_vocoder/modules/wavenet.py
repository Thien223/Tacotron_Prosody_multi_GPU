		...
		if hparams.upsample_conditional_features:
			...
			for i, s in enumerate(hparams.upsample_scales):
				with tf.variable_scope('local_conditioning_upsampling_{}'.format(i+1)):
					convt = ConvTranspose2d(1, (hparams.freq_axis_kernel_size, 2 * s),padding='same', strides=(1, s))
		...
		with tf.variable_scope('inference') as scope:
			...
			if self.is_training:
				...
				if is_mulaw_quantize(hparams.input_type):
					...
					y_hat_log = inv_mulaw_quantize(y_hat_log, hparams.quantize_channels)
					y_log = inv_mulaw_quantize(y_log, hparams.quantize_channels)
					...
					if is_mulaw(hparams.input_type):
						y_hat_log = inv_mulaw(y_hat_log, hparams.quantize_channels)
			...
			#synthesizing
				...
				if is_mulaw_quantize(hparams.input_type):
					...
					y_hat = inv_mulaw(tf.reshape(y_hat, [batch_size, -1]), hparams.quantize_channels)
				else:
