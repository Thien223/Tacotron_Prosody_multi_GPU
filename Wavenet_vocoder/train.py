...
import time
import traceback
from Wavenet_vocoder.modules.wavenet import wavenet
...
from Utils.Utils import ValueWindow, waveplot
from Utils.Wavenet_feeder import Feeder
...
def eval_step(sess, global_step, model, plot_dir, wav_dir, summary_writer, hparams):
    ...
    # Save figure
    waveplot(plot_path, y_hat, y_target, model._hparams)
...
def model_train_mode(args, feeder, hparams, global_step):
    with tf.variable_scope('model', reuse=tf.AUTO_REUSE) as scope:
        print('train.py link 117 ####### feeder.targets.shape : {}'.format(feeder.targets.shape))
        print('train.py link 117 ####### feeder.targets : {}'.format(feeder.targets))
...
def GetArguments():
    ...
    parser.add_argument('--wavenet_train_steps', type=int, default=300000,
